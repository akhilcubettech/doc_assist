...
# Debug Section
if st.sidebar.checkbox("Show System Diagnostics"):
    st.sidebar.subheader("System Diagnostics")
    st.sidebar.write(f"Context Length: {len(st.session_state.processed_data['analysis_context'])} chars")
    st.sidebar.write(f"Processed Images: {len(st.session_state.processed_data['images'])}")
    st.sidebar.write(f"Chat History: {len(st.session_state.chat_history)} messages")
